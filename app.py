...
try:
    ...
    st.error(f"❌ ERROR: No se pueden cargar módulos de IA: {e}")
    st.error("Esto indica un problema con las dependencias de PyTorch.")
    st.error("Contacte soporte de Hugging Face Spaces.")
    st.stop()
except Exception as e:
    st.error(f"❌ ERROR CRÍTICO en módulos: {e}")
    st.error("La aplicación no puede continuar.")
    st.stop()
